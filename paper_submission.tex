...
\begin{itemize}[nosep]
    \item While validated across two models (DeepSeek-chat and Qwen3-14B), more models are needed for generalization.
...
We presented a controlled comparison across two models. Key findings:
egin{enumerate}[nosep]
    \item Citation reliability varies across models (59.1% DS vs 16.4% Qwen3), but workflow ranking is consistent.
    \item Simulated retrieval performs worst across both models.
    \item Cautious prompting reduces hallucination but increases no-DOI rate.
    \item Chinese prompts underperform English.
    \item Cross-model invariance suggests robust effects.
\end{enumerate}
