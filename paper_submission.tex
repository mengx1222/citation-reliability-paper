...
\begin{enumerate}[nosep]
    ...
    \item \wthree~(Verify-and-Repair) performed comparably to \wzero~(60.3\% vs.~64.5\%), with overlapping confidence intervals, suggesting limited additional benefit over direct generation.
...
The most striking finding is the gap in verification rates: DeepSeek-chat achieves 59.1\% overall resolution, whereas Qwen3-14B resolves only 16.4\% of its 1,024 generated references. Despite this absolute difference, the relative ranking of workflows is consistent: \wthree~and \wzero~achieve the highest verification rates, followed by \wone, with \wtwo~worst. The no-DOI rate is substantially higher for Qwen3 across all workflows, suggesting that Qwen3 is more cautious about fabricating DOIs without improving overall verification. The invariant ranking across models---with direct generation and verify-and-repair as the top two comparable workflows---suggests that these effects are a robust property of current LLM behavior rather than a model-specific artifact.
...
The gap between DeepSeek (59.1%) and Qwen3 (16.4%) warrants examination. The invariant workflow ranking (with W3 and W0 as the top two workflows) across both models suggests these effects are not model-specific.
